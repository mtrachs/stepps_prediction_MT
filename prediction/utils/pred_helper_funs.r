...
build_weight_matrix <- function(post, d, idx_cores, N, N_cores, run){
  ...
  col_names <- colnames(post[,1,]) ## added by MT
  par_names  = unlist(lapply(col_names, function(x) strsplit(x, "\\[")[[1]][1]))
  
  KW     = FALSE
  kernel = run$kernel
  phi    = colMeans(post[,,which(par_names == 'phi')])
  
  if (kernel=='gaussian'){
    one_psi = run$one_psi
    if (one_psi){
      #       psi   = rep(mean(post[,1,which(par_names == 'psi')]), K)
      psi = mean(post[,,which(par_names == 'psi')])
    } else {
      KW  = TRUE
      psi = colMeans(post[,,which(par_names == 'psi')])
    }
  } else if (kernel=='pl'){
    one_a = run$one_a
    if (one_a){
      #       a = rep(mean(post[,1,which(par_names == 'a')]), K)
      a = median(post[,,which(par_names == 'a')])
    } else {
      KW = TRUE
      a  = apply(post[,,which(par_names == 'a')],2,median)
    }
    
    one_b = run$one_b
    if (one_b){
      #       b = rep(mean(post[,1,which(par_names == 'b')]), K)
      b = mean(post[,,which(par_names == 'b')])
      if (KW) b = rep(b, K)
    } else {
      KW = TRUE
      b  = colMeans(post[,,which(par_names == 'b')])
    }
  }
