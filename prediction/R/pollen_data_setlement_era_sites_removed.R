...
total.exclude <- ((min.dist.lakes > 10000)|((long.real> (-68)))|((long.real < (-69.5))& (lat.real>45.25))|
                    ((lat.real < 39.8))|((long.real < (-75))& (lat.real>44.25)))
...
source(paste(help.fun.loc,'taxa_translation.R',sep=''))
...
stan_rdump(list = names(test_neus), file = paste('~/workflow_stepps_prediction/prediction/data/elicitation_neus_certainty_median_',dim(y)[1],'_sites.dump',sep=''))

veg_coords <- veg_table@coords
pollen_coords <- pol_table@coords
save(list = c(names(test_neus),'veg_coords','pollen_coords'), 
     file = paste('~/workflow_stepps_prediction/prediction/data/elicitation_neus_certainty_median_',dim(y)[1],'_sites.RData',sep=''))
#-------------------------------------------------------------------------------------------------------------------
#also save data as .csv

write.csv(calib_trans,paste(data.loc,'pollen_elicitation_NEUS_',dim(y)[1],'_sites.csv',sep=''))
write.csv(veg_mean,paste('~/workflow_stepps_prediction/prediction/data/vegetation_mean_NEUS_',dim(y)[1],'_sites.csv',sep=''))