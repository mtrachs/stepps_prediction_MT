...
calib_trans <- readRDS('~/workflow_stepps_calibration/calibration/data/settlement_era_pollen.RDS')
y <- round(calib_trans[,colnames(calib_trans)%in%target_taxa])
...
stan_rdump(
  ...
  file=paste('~/workflow_stepps_prediction/prediction/data/prediction_test2.dump',sep=""))
...
save(K, N,T, N_knots, N_cores,lag, 
     ...
     d,d_knots,d_inter,w,
     coord.agg.final, 
     file=paste('~/workflow_stepps_prediction/prediction/data/prediction_test2.rdata',sep=""))
